...
const EditServiceModal = ({ 
  ...
  editService, 
  setEditService, 
  currentEditSubService, 
  setCurrentEditSubService, 
  handleAddEditSubService, 
  handleRemoveEditSubService, 
  handleUpdateService 
}) => {
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <div className="flex space-x-2 mb-3">
              <input
                type="text"
                value={currentEditSubService}
                onChange={(e) => setCurrentEditSubService(e.target.value)}
                ...
                onKeyPress={(e) => e.key === 'Enter' && handleAddEditSubService()}
              />
              <button
                onClick={handleAddEditSubService}
                ...
                <ul className="space-y-1">
                  {editService.subServices.map((subService, index) => (
                    <li key={index} className="flex justify-between items-center bg-gray-50 px-2 py-1 rounded">
                      ...
                      <button
                        onClick={() => handleRemoveEditSubService(index)}
